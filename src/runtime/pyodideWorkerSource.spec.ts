...
function createPyodideWorkerHarness(options?: { mockPyodideInstance?: any }) {
  ...
  const mockPyodide = options?.mockPyodideInstance || {
    FS: mockFileSystem,
  ...
  (globalThis as any).loadPyodide = mockPyodide ? async () => mockPyodide : undefined;

  const workerFactory = new Function('self', PYODIDE_WORKER_SCRIPT) as (
    self: unknown,
  ) => void;
  workerFactory(shim);
...
describe('PYODIDE_WORKER_SCRIPT System / FS and IPython Engine', () => {
